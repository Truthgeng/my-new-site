-- Atomic credit check + deduction function
-- Returns true if a credit was successfully deducted, false if the user is out of credits.
-- Uses SELECT FOR UPDATE to prevent parallel calls from racing past 0.
...
BEGIN
  ...
  IF rec.last_credit_deduction IS NOT NULL AND
    ...
    -- It's a parallel duplicate call for the same pitch — let it through without double-deducting
